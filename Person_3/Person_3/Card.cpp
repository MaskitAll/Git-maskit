...
Card& Card::operator=(Card card) {
	return Card(card.Name, card.ID, card.lvl, card.Rhand, card.Lhand, card.HP, card.Armor, card.Speed, card.race, card.elements, card.Description);
}
...
std::string Card::Card_to_filestring() {
	...
	res += smth_to_filestring(Name, 14);
...
void Card::filestring_to_Card(std::string filestring) {
	...
	setID(stoi(filestring.substr(pos, pos1)));
	pos = pos1 + 2;

	pos1 = filestring.find("|", pos);
	setName(filestring.substr(pos, pos1));
	pos = pos1 + 2;

	for (int i = 1; i <= 6; ++i) {
		pos1 = filestring.find("|", pos);
		setCharacter(*int_to_Character(i), stoi(filestring.substr(pos, pos1)));
		pos = pos1 + 2;
	}

	pos1 = filestring.find("|", pos);
	race = string_to_Race(filestring.substr(pos, pos1));
	pos = pos1 + 2;

	pos1 = filestring.find("|", pos);
	elements = string_to_Elements(filestring.substr(pos, pos1));
	pos = pos1 + 2;

	pos1 = filestring.find("|", pos);
	Description = filestring.substr(pos, pos1);
}
